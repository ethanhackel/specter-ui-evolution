      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map((feature, index) => (
            <motion.div
              key={feature.title}
              className="glass-card-hover p-7 group cursor-default relative overflow-hidden"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              {feature.badge && (
                <span className="absolute top-4 right-4 text-[0.6rem] font-mono tracking-widest px-2 py-0.5 rounded-sm bg-primary/10 text-primary border border-primary/20">
                  {feature.badge}
                </span>
              )}
              <div className="w-12 h-12 rounded-lg bg-accent flex items-center justify-center mb-5 group-hover:bg-primary/20 transition-colors">
