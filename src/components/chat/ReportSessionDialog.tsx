...
const ReportForm = ({ reason, setReason, onSubmit, onCancel }: {
  reason: string;
  setReason: (v: string) => void;
  onSubmit: () => void;
  onCancel: () => void;
}) => (
  <div className="space-y-3 sm:space-y-4 pt-1 px-1">
    <div>
    ...
    return (
      <Drawer open={open} onOpenChange={onOpenChange}>
        <DrawerContent className="border-border bg-card px-4 pb-6">
          ...
          <ReportForm reason={reason} setReason={setReason} onSubmit={handleSubmit} onCancel={handleCancel} />
        </DrawerContent>
      </Drawer>
    );
  }
